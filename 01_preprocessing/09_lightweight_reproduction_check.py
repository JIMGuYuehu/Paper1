#!/usr/bin/env python3
"""Run small raw-to-existing-product numerical checks on STREAM.

INPUT
    Immutable WACCM, MERRA-2 and combined EXTR ClOx archives below
    /mnt/backup_ETH and /mnt/soclim0/public_data/weiji, plus the established
    read-only processed products below /mnt/soclim0/public_data/weiji.
OUTPUT
...
RUNTIME_ROOT = (PAPER1_ROOT / "runtime").resolve()
DIAGNOSTIC_LIB = PAPER1_ROOT / "02_diagnostics" / "lib"
...
from paper1_noleap import encoding_for, normalize_dataset  # noqa: E402


BACKUP_ROOT = Path(os.environ.get("PAPER1_BACKUP_ROOT", "/mnt/backup_ETH")).resolve()
PUBLIC_ROOT = Path(
    os.environ.get("PAPER1_ARCHIVE_ROOT", "/mnt/soclim0/public_data/weiji")
).resolve()
DERIVED_ROOT = Path(
    os.environ.get("PAPER1_DERIVED_ROOT", str(RUNTIME_ROOT))
).resolve()
...
def assert_safe_output() -> None:
    """Confine all writes to this checkout's ignored Paper1/runtime tree."""

    require(RUNTIME_ROOT == PAPER1_ROOT.joinpath("runtime"), "runtime is a symlink")
    require(
        DERIVED_ROOT == RUNTIME_ROOT or RUNTIME_ROOT in DERIVED_ROOT.parents,
        f"PAPER1_DERIVED_ROOT must remain below {RUNTIME_ROOT}",
    )
    require(
        OUTPUT_ROOT == DERIVED_ROOT or DERIVED_ROOT in OUTPUT_ROOT.parents,
        "validation output escapes PAPER1_DERIVED_ROOT",
    )
    for protected in (BACKUP_ROOT, PUBLIC_ROOT):
        require(
            OUTPUT_ROOT != protected and protected not in OUTPUT_ROOT.parents,
            f"validation output overlaps protected source root: {protected}",
        )
    OUTPUT_ROOT.mkdir(parents=True, exist_ok=True)
    require(OUTPUT_ROOT.resolve() == OUTPUT_ROOT, "validation output is a symlink")
...
def compare_merra_daily() -> list[dict[str, object]]:
    ...
    cases = (
        (
            sorted(PUBLIC_ROOT.glob("MERRA2M2I6NPANA/MERRA2_*.20200101.SUB.nc"))[0],
            PUBLIC_ROOT / "MERRA2_Processed" / "O3" / "MERRA2.O3.2020.nc",
            "O3",
            "O3",
        ),
        (
            sorted(PUBLIC_ROOT.glob("MERRA2M2I6NPANA/Z/MERRA2_*.20200101.SUB.nc"))[0],
            PUBLIC_ROOT / "MERRA2_Processed" / "Z3" / "MERRA2.Z3.2020.nc",
            "H",
            "Z3",
        ),
    )
...
def compare_partial_ozone() -> list[dict[str, object]]:
    ...
    source_path = PUBLIC_ROOT / "BWCN" / "O3" / "BWCN.cam.h3.0008.O3.nc"
    reference_path = PUBLIC_ROOT / "BWCN" / "partial_O3" / "BWCN_partial_O3_all_ranges.nc"
...
def main() -> None:
    ...
    long_raw = sorted(
        (BACKUP_ROOT / "extr_2000" / "extr_2000" / "B2000WCN.e122.f19_g16.001" / "atm" / "hist").glob(
    ...
    results.extend(
        compare_waccm_chunk(
            long_raw,
            PUBLIC_ROOT / "B2000WCN001002_timefixed" / "O3" / "B2000WCN.sample.cam.h3.0001.O3.nc",
            ("O3",),
    ...
    results.extend(
        compare_waccm_chunk(
            long_raw,
            PUBLIC_ROOT / "B2000WCN001002_timefixed" / "U" / "B2000WCN.sample.cam.h3.0001.U.nc",
            ("U",),
            "WACCM_LONGRUN:run001:year0001:first_chunk",
        )
    )
    ...
    bwcn_raw = sorted(
        (BACKUP_ROOT / "LENS" / "BWCN.e122.f19_g16.002" / "atm" / "hist").glob(
    ...
    results.extend(
        compare_waccm_chunk(
            bwcn_raw,
            PUBLIC_ROOT / "BWCN" / "O3" / "BWCN.cam.h3.0008.O3.nc",
            ("O3",),
            "WACCM_BWCN:year0008:first_chunk",
        )
    )

    hindcast_files = sorted((BACKUP_ROOT / "lens" / "0008-01").glob("*.cam.h3.*.nc*"))
    ...
    results.extend(
        compare_waccm_chunk(
            ...
            PUBLIC_ROOT / "Hindcast" / "0008-01" / "O3" / f"{member_name}.O3.nc",
            ("O3",),
