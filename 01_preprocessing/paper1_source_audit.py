...
INPUT
    WACCM restart chunks/pressure products, Marina members, MERRA-2 SUB or
...
MARINA_REQUIRED_LEVELS_PA = np.asarray(
    [1000.0, 3000.0, 5000.0, 7000.0, 10000.0, 15000.0, 100000.0]
)
MARINA_CANONICAL_23_PA = np.asarray(
    [
...
def marina_id(path: Path, case: str) -> str:
    marker = f".{case}."
    if marker not in path.name or not path.name.endswith(".nc"):
        raise ValueError(f"unexpected Marina filename: {path.name}")
    return path.name.split(marker, 1)[1][:-3]


def marina(args: argparse.Namespace) -> None:
    ...
    for case, directory in case_dirs.items():
        ...
        if len(found) != 30:
            raise RuntimeError(f"expected 30 Marina {case} files; found {len(found)}")
        mapped = {marina_id(path, case): path for path in found}
        if len(mapped) != 30:
            raise RuntimeError(f"duplicate Marina member IDs in {case}")
    ...
    if len(common) != 30 or set(paths_by_case["0008-02"]) != set(paths_by_case["0008-03"]):
        raise RuntimeError("Marina February and March do not share the same 30 member IDs")
    ...
    for case in ("0008-02", "0008-03"):
        ...
        for member_id in common:
            ...
            with netCDF4.Dataset(path) as dataset:
                ...
                if levels_pa.shape != MARINA_CANONICAL_23_PA.shape or not np.allclose(
                    levels_pa, MARINA_CANONICAL_23_PA, rtol=0.0, atol=1e-6
                ):
                    raise RuntimeError(
                ...
                absent = [
                    float(level)
                    for level in MARINA_REQUIRED_LEVELS_PA
    ...
    print(f"validated Marina Feb=30, Mar=30, common=30; wrote {args.output}")
...
def source_mode(
    ...
    with netCDF4.Dataset(path) as dataset:
        ...
        if ntime == 1:
            ...
            # GES DISC L34RS daily-subset files on STREAM retain the explicit
