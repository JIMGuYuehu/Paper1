...
#     ${PAPER1_ARCHIVE_ROOT}/MERRA2/M2I6NPANA
#   MERRA-2 geopotential-height source H granules:
#     ${PAPER1_ARCHIVE_ROOT}/MERRA2/M2I6NPANA/Z
# OUTPUT
...
CORE_ROOT="${PAPER1_MERRA2_SOURCE:-${PAPER1_ARCHIVE_ROOT}/MERRA2/M2I6NPANA}"
...
command -v "${CDO_BIN}" >/dev/null 2>&1 || die "CDO executable not found: ${CDO_BIN}"
prepare_output_dir "${OUT_ROOT}"
prepare_output_file "${MANIFEST}"

python "${SCRIPT_DIR}/source_inventory.py" merra \
    --core-root "${CORE_ROOT}" \
    --z-root "${Z_ROOT}" \
    --start-year 1980 \
    --end-year 2025 \
...
python "${SCRIPT_DIR}/source_inventory.py" merra-yearly \
    --root "${OUT_ROOT}" \
    --start-year 1980 \
    --end-year 2025 \
